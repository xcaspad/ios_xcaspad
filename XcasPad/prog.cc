...
namespace giac {
  ...
  void check_local_assign(const gen & g,const vecteur & prog_args,vecteur & res1,vecteur & res2,vecteur & res3,vecteur & res4,bool testequal,GIAC_CONTEXT){
	...
	*logptr(contextptr) << "Warning, =< is in-place assign, check " << v << endl;
    }
  ...
  symbolic symb_program_sto(const gen & a,const gen & b,const gen & c,const gen & d,bool embedd,GIAC_CONTEXT){
	...
	*logptr(contextptr) << gettext("Warning, defining a derivative function should be done with function_diff or unapply: ") << c << endl;
	...
	if (inters.type==_VECT && !inters._VECTptr->empty()){
	  ...
	  *logptr(contextptr) << "Warning: Local variables shadow function arguments " << inters << endl;
	}
      }
    }
    return g;
  ...
  gen ifte(const gen & args,bool isifte,const context * contextptr){
	...
	if (isifte){
	  gensizeerr("Ifte: Unable to check test",res); 
	  return res;
	}
	else
  ...
  // loop
  static string printasfor(const gen & feuille,const char * sommetstr,GIAC_CONTEXT){
    if (feuille.type!=_VECT) 
      return sommetstr+('('+feuille.print(contextptr)+')');
	...
	if (jt->_SYMBptr->sommet==at_check_type){
	  ...
	  if (jt->_SYMBptr->sommet==at_double_deux_points ){
	    ...
	    if (jt->_SYMBptr->sommet==at_of){
	      tmp=jt->_SYMBptr->feuille._VECTptr->front();
	      *logptr(contextptr) << "Invalid variable "+jt->print(contextptr)+" using "+tmp.print(contextptr)+" instead.";
	    }
	    else
	      tmp=*jt;
	  }
...
#else
	  *logptr(contextptr) << "Reserved word:"+tmp.print(contextptr) << endl;
#endif
	  return -RAND_MAX;
	}
	else {
...
#else
	  *logptr(contextptr) << "Not bindable"+tmp.print(contextptr) << endl;
#endif
	  return -RAND_MAX;
	}
      }
	...
	*logptr(contextptr) << "Unknown subsop rule " << *it << endl;
	...
	if (rec.size()==1)
	  ...
	  if (rec.front().type!=_INT_ || i<0 || i>=signed(res.size()))
	    return gendimerr(contextptr);
	  if (is_undef( (res[i]=subsop(res[i],vecteur(1,symbolic(at_equal,makevecteur(vecteur(rec.begin()+1,rec.end()),w.back()))),contextptr)) ) )
	    return res[i];
	  continue;
	}
      }
...
#else
	*logptr(contextptr) << "Unable to sort " << c << endl;
#endif
	return true;
      }
      return !is_zero(c);
  ...
  */
  ...
  gen _tablefunc(const gen & args,GIAC_CONTEXT){
    ...
    gen l32(symb_evalf(symbolic(at_subst,makevecteur(_cell(makevecteur(zero,vecteur(1,zero)),contextptr),_cell(makevecteur(zero,vecteur(1,minus_one)),contextptr),_cell(makevecteur(vecteur(1,zero),vecteur(1,minus_one)),contextptr)))));
  ...
  gen _tableseq(const gen & args,GIAC_CONTEXT){
	...
	res.push_back(l31);
      ...
      res.push_back(uzero);
      gen l51(symb_evalf(symbolic(at_subst,makevecteur(_cell(makevecteur(zero,vecteur(1,zero)),contextptr),_cell(makevecteur(plus_one,vecteur(1,zero)),contextptr),_cell(makevecteur(vecteur(1,minus_one),vecteur(1,zero)),contextptr)))));
	...
	tmp2.push_back(_cell(makevecteur(vecteur(1,i-dim),vecteur(1,zero)),contextptr));
      ...
      gen l41(symb_eval(symbolic(at_subst,makevecteur(_cell(makevecteur(zero,vecteur(1,zero)),contextptr),tmp1,tmp2))));
  ...
  gen protecteval(const gen & g,int level, GIAC_CONTEXT){
	...
	*logptr(contextptr) << "Restoring epsilon to 1e-6 from " << v[0] << endl;
	epsilon(1e-6,contextptr);
      }
      if (is_strictly_greater(v[1],1e-6,contextptr)){
	*logptr(contextptr) << "Restoring proba epsilon to 1e-6 from " << v[0] << endl;
  ...
  */
  ...
  static const char _regrouper_s []="regrouper";
  ...
  */
  ...
  const unsigned long piecewise_tab_alias[]={alias_at_piecewise,0};
