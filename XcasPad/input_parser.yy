...
exp	: T_NUMBER		{$$ = $1;}
	| T_NUMBER T_SYMBOL 	{if (is_one($1)) $$=$2; else $$=symbolic(at_prod,makevecteur($1,$2));}
	| T_NUMBER T_SYMBOL T_POW exp	{if (is_one($1)) $$=symb_pow($2,$4); else $$=symbolic(at_prod,makevecteur($1,symb_pow($2,$4)));}
	| T_NUMBER T_SYMBOL T_SQ	{$$=symbolic(at_prod,makevecteur($1,symb_pow($2,$3)));}
	| T_NUMBER T_LITERAL 	{if (is_one($1)) $$=$2; else $$=symbolic(at_prod,makevecteur($1,$2));}
	...
	| symbol T_BEGIN_PAR suite T_END_PAR T_AFFECT exp {if (lidnt($3).empty() || (abs_calc_mode(giac_yyget_extra(scanner))==38 && $1.type==_IDNT && strlen($1._IDNTptr->id_name)==2 && check_vect_38($1._IDNTptr->id_name))) $$=symbolic(at_sto,makevecteur($6,symbolic(at_of,makevecteur($1,$3)))); else $$ = symb_program_sto($3,$3*zero,$6,$1,true,giac_yyget_extra(scanner));}
	| exp TI_STO symbol T_BEGIN_PAR suite T_END_PAR {if (lidnt($5).empty() || (abs_calc_mode(giac_yyget_extra(scanner))==38 && $3.type==_IDNT && check_vect_38($3._IDNTptr->id_name))) $$=symbolic(at_sto,makevecteur($1,symbolic(at_of,makevecteur($3,$5)))); else $$ = symb_program_sto($5,$5*zero,$1,$3,false,giac_yyget_extra(scanner));}
	...
	| exp TI_STO symbol T_INDEX_BEGIN T_VECT_DISPATCH exp T_VECT_END T_VECT_END { 
         const giac::context * contextptr = giac_yyget_extra(scanner);
         gen g=symbolic(at_of,makevecteur($3,$6)); $$=symb_sto($1,g); 
        }
	| exp TI_STO symbol { if ($3.type==_IDNT && unit_conversion_map().find($3.print(context0).c_str()+1) != unit_conversion_map().end()) $$=symbolic(at_convert,makevecteur($1,symbolic(at_unit,makevecteur(1,$3)))); else $$=symb_sto($1,$3); }
	| exp TI_STO T_UNARY_OP { $$=symbolic(at_convert,makevecteur($1,$3)); }
	| exp TI_STO T_PLUS { $$=symbolic(at_convert,makevecteur($1,$3)); }
	| exp TI_STO T_FOIS { $$=symbolic(at_convert,makevecteur($1,$3)); }
	| exp TI_STO T_DIV { $$=symbolic(at_convert,makevecteur($1,$3)); }
  ...
  $$ =symbolic(*$2._FUNCptr,makevecteur($1,$3));}
	...
	| exp T_INTERVAL exp	{$$ = symbolic(*$2._FUNCptr,makevecteur($1,$3)); }
	...
	| T_FOR symbol from T_TO exp step loop38_do prg_suite T_BLOC_END { 
          ...
          const giac::context * contextptr = giac_yyget_extra(scanner);
	  if (!lidnt($6).empty())
            *logptr(contextptr) << "Warning, step is not numeric " << $6 << std::endl;
          bool b=has_evalf($6,tmp,1,context0);
          if (!b || is_positive(tmp,context0)) 
             $$=symbolic(*$1._FUNCptr,makevecteur(symb_sto($3,$2),symb_inferieur_egal($2,$5),symb_sto(symb_plus($2,b?abs($6,context0):symb_abs($6)),$2),symb_bloc($8))); 
          else 
            $$=symbolic(*$1._FUNCptr,makevecteur(symb_sto($3,$2),symb_superieur_egal($2,$5),symb_sto(symb_plus($2,$6),$2),symb_bloc($8))); 
        }
	| T_FOR symbol from step T_TO exp T_DO prg_suite T_BLOC_END { 
	 ...
	 if (!lidnt($4).empty())
            *logptr(contextptr) << "Warning, step is not numeric " << $6 << std::endl;
         bool b=has_evalf($4,tmp,1,context0);
         if (!b || is_positive(tmp,context0)) 
           $$=symbolic(*$1._FUNCptr,makevecteur(symb_sto($3,$2),symb_inferieur_egal($2,$6),symb_sto(symb_plus($2,b?abs($4,context0):symb_abs($4)),$2),symb_bloc($8))); 
         else 
           $$=symbolic(*$1._FUNCptr,makevecteur(symb_sto($3,$2),symb_superieur_egal($2,$6),symb_sto(symb_plus($2,$4),$2),symb_bloc($8))); 
        } 
	| T_FOR symbol from step T_DO prg_suite T_BLOC_END { $$=symbolic(*$1._FUNCptr,makevecteur(symb_sto($3,$2),plus_one,symb_sto(symb_plus($2,$4),$2),symb_bloc($6))); }
	| T_FOR symbol from step T_MUPMAP_WHILE exp T_DO prg_suite T_BLOC_END { $$=symbolic(*$1._FUNCptr,makevecteur(symb_sto($3,$2),$6,symb_sto(symb_plus($2,$4),$2),symb_bloc($8))); }
	| T_FOR {$$ = gen(*$1._FUNCptr,4);}
	| T_DO prg_suite T_BLOC_END { vecteur v=makevecteur(zero,plus_one,zero,symb_bloc($2)); $$=symbolic(*$1._FUNCptr,v); }
	...
	| T_IFERR prg_suite T_THEN prg_suite T_ELSE prg_suite T_BLOC_END {$$=symbolic(at_try_catch,makevecteur(symb_bloc($2),0,symb_bloc($4),symb_bloc($6)));}
	...
	| exp T_DOLLAR_MAPLE symbol T_IN exp {$$=symb_dollar(makevecteur($1,$3,$5));}
	...
	| exp T_INDEX_BEGIN T_VECT_DISPATCH exp T_VECT_END T_VECT_END	{
          const giac::context * contextptr = giac_yyget_extra(scanner);
	  $$ = symbolic(at_of,makevecteur($1,$4));
        }
	...
	| T_MUPMAP_WHILE exp T_DO prg_suite T_BLOC_END { $$=symbolic(*$1._FUNCptr,makevecteur(zero,equaltosame($2),zero,symb_bloc($4))); }
...
ti_bloc_end:	T_BLOC_END { $$=0; }
	     |	TI_DEUXPOINTS T_BLOC_END { $$=0; }
	     ;
